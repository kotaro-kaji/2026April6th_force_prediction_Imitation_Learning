...
This is useful as a structural precedent for predicting future force or contact-relevant state before control.
...
Your current preferred framing is:
...
1. Take the history-encoder intuition from `dynamical-metalearning`.
2. Compress the inferred hidden dynamics into an explicit latent `d_i` or `z_env`.
3. Use that latent inside a force predictor or policy.
4. Update only the latent online at deployment.
...
4. If you want a simple first version, implement a small Transformer history encoder plus latent head plus residual dynamics or force-prediction head.
...
- [Research Theme 1: Force Prediction and Material Latent for Imitation Learning](research_theme_1_force_prediction_material_imitation.md)
